...
function [integration_error, Nt, ft_5, integral_1000] = zadanie3()
    ...
    ft_5 = f(5);
    integral_1000 = Simpson_integration(@f, 1000);

    for i = 1:length(Nt)
        integration_result = Simpson_integration(@f, Nt(i));
...
function integral = Simpson_integration(f, k)
    % Zakres całkowania
    a = 0;
    b = 5;
    ...
    sum_y = 0;
    for i = 1:k
        sum_y = sum_y + f(x(i)) + 4 * f((x(i+1) + x(i)) / 2) + f(x(i+1));
    end
